...
// GetWithCategory retrieves an Place by its ID with associated category.
func (r *GormPlaceRepository) GetWithCategory(placeID string) (model.Place, error) {
	var place model.Place
	if err := r.DB.Preload("PlaceCategory").First(&place, placeID).Error; err != nil {
		return place, err
	}
	return place, nil
}
...
func (r *GormPlaceRepository) GetAllWithCategory() ([]model.Place, error) {
	var activities []model.Place
	if err := r.DB.Preload("PlaceCategory").Find(&activities).Error; err != nil {
		return nil, err
	}
	return activities, nil
}
