...
// Export a standalone function to download Excel without UI
export const downloadExcel = (travelDetail: TravelDetailData) => {
  ...
  // Generate a better filename with fallbacks
  const destinationName = travelDetail.destination || 
                          travelDetail.trip_name || 
                          `trip-${travelDetail.id}` || 
                          `travel-plan-${new Date().toISOString().slice(0, 10)}`;
  
  XLSX.writeFile(wb, `lich-trinh-${destinationName}.xlsx`);
  message.success(
